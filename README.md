...
- **等效产出口径** — 单条不设 8h 上限（计价表 v1.1 起），> 8h 向上靠 4h 档取整、不再拆分；已完成任务只用边界遗漏不算乘法返工
...
### 3. 直接写入 → 写后审计块
算完不等确认，直接写入 `日报需求记录-{YYYY}-{MM}-{DD}.md`；写后在回复里输出「写后审计块」（已跳过 commit 清单、合并结果、五维度单价 × 数量算式、工时汇总、压缩比）供事后核对，有异议直接改 md 对应行。当日无本人 commit 则回复无可记录提交收工，不编造记录。
