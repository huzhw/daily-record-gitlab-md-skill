...
- **AI 工时 = 单价表机械求和的等效投入**（计价表 v1.5 定口径，v1.3 统一四舍五入到 0.5h）— 需求先按机械判据分型（简单修复/常规/复杂/新架构）锁设计协助档，生成 1min/文件+1min/百行、审查 3min/文件（与生成同一文件集）、修正对话仅计代码返工轮 2min/轮；🔴 禁止用当日提交时间窗/会话时长反推压缩，AI ≤ 人工 是唯一护栏
...
- **等效产出口径** — 人工工时单条不设 8h 上限（v1.1 起），人工/AI 取整统一四舍五入到 0.5h（v1.3）、不再拆分；AI 列同为等效投入、不与日历耗时挂钩（v1.4）；子项切分与 ×0.6 折扣走 v1.5 机械规则（业务链路簇 + 源码 insertions 定锚）；已完成任务只用边界遗漏不算乘法返工
...
### 2. 分型 → 查链路卡 → 五维工时评估
需求先按机械判据分型（简单修复/常规/复杂/新架构，锁 AI 设计协助档）；数量口径查 chains.md 对应链路卡（Java 一个类、React 一个组件、JSP 一个页面块、Python 一个函数各自算「一项」）；人工五维单价不变，AI 辅助工时按 AI 单价表机械求和另算（口径 v1.5，不按日历窗口修正；数量口径见 chains.md「AI 计价数量口径」）。
