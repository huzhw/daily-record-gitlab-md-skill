...
- **只读自己的提交** — `today_commits.py` 一键汇总当天提交：内置 `--author="胡志伟"` 过滤，一次输出提交清单/文件数/行数（逐 commit numstat 求和）/时间窗/分支/GitLab 网页链接 JSON，同分支上同事提交不混入
...
- **AI 工时 = 单价表机械求和的等效投入**（计价表 v1.4 定口径，v1.3 统一四舍五入到 0.5h）— 需求先按机械判据分型（简单修复/常规/复杂/新架构）锁设计协助档，生成 1min/文件+1min/百行、审查 3min/文件（与生成同一文件集）、修正对话仅计代码返工轮 2min/轮；🔴 禁止用当日提交时间窗/会话时长反推压缩，AI ≤ 人工 是唯一护栏
...
### 1. 汇总提交 → 增量过滤 → 合并任务
跑 `today_commits.py` 拉取当天全部提交（内置 `--author="胡志伟"` 过滤）；跑 `recorded_commits.py` 与当天 md 已记录的（仓库 + 提交id）机械比对去重；未记录的按模块/功能聚拢合并，同文件反复改的、同模块同类改动的、同一功能全链路的全部合为一条需求。

### 2. 分型 → 查链路卡 → 五维工时评估
需求先按机械判据分型（简单修复/常规/复杂/新架构，锁 AI 设计协助档）；数量口径查 chains.md 对应链路卡（Java 一个类、React 一个组件、JSP 一个页面块、Python 一个函数各自算「一项」）；人工五维单价不变，AI 辅助工时按 AI 单价表机械求和另算（口径 v1.4，不按日历窗口修正；数量口径见 chains.md「AI 计价数量口径」）。
...
### 3. 直接写入 → 写后审计块
算完不等确认，直接写入 `日报需求记录-{YYYY}-{MM}-{DD}.md`；写后在回复里输出「写后审计块」（已跳过 commit 清单、合并结果、五维度单价 × 数量算式、AI 单价表算式（机械求和）+ 会话轮数证据行、工时汇总、压缩比）供事后核对，有异议直接改 md 对应行；审计块同步以 HTML 注释落盘 md 末尾，下次触发自动回看比对（自我校准）。当日无本人 commit 则回复无可记录提交收工，不编造记录。
...
- 需求概述末行溯源括号必带 **GitLab 网页链接 + 分支 + commit id + 提交日期**（如 `（2次提交 09-05 13:42~13:52 · 分支 master · GitLab：https://gitlab.xx.com/g/lanxum-amisp · commit 1a2b3c/4d5e6f）`），来源为 `today_commits.py` 输出的 `web_url` / `branch` / `commits[].datetime`（内部即 `git remote get-url origin` + `git branch --show-current` + `git log %ai`，ssh 自动转网页链接）
